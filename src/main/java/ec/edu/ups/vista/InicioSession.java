...
public class InicioSession extends javax.swing.JInternalFrame {
    ...
    public InicioSession(MenuPrincipal menuPrincipal, ControladorDocente controladorDocents, ControladorRector controladorRector) {
        initComponents();
        menup=menuPrincipal;
        controladorD=controladorDocents;
    ...
    private void btnIniciarSessionActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnIniciarSessionActionPerformed
        if(!txtCorreo.getText().isEmpty()&&!txtContrasenia.getText().isEmpty()){
            ...
            rector=controladorR.login(correo, contrasenia);
            if(rector!=null){
                menup.getMenuRector().setVisible(true);
                ...
                if(docente!=null){
                menup.getMenuDocente().setVisible(true);
                menup.getCerrarSession().setVisible(true);
                menup.getRegistrarseTab().setVisible(false);
                menup.getIniciarsessionTab().setVisible(false);
                JOptionPane.showMessageDialog(null, "BIENVENIDO");
